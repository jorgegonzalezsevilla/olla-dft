    ...
    for sym, p in common["pseudos"].items():
        ...
        if rel != "full":
            malos.append(f"{sym} ({p['filename']}: relativistic="
                         f"{rel or 'not declared'})")
    if malos:
        raise ErrorDeUso(
            "spin-orbit coupling needs FULLY RELATIVISTIC "
            "pseudopotentials (relativistic='full'), and these are not:"
            "\n  " + "\n  ".join(malos) +
            "\n\nWith scalar-relativistic pseudos, lspinorb does not fail: "
            "it returns a spin-orbit\nsplitting of zero that looks like "
            "a valid result. Look for the corresponding 'rel-'\n"
            "pseudos (for example rel-pbe in the SSSP table or in "
            "pslibrary)."
        )
...
def write_run_script(jobs: list, path: Path, nproc: int = None,
    ...
    lines = [
        "#!/bin/bash",
        "# Generated by Olla-DFT — runs every point of the sweep.",
        "#   ./run.sh      one after another",
        "#   ./run.sh 4    four at a time",
        "# On Windows, or if you prefer not to depend on bash:  python run.py",
        "PAR=${1:-1}",
        ...
        'PW=${PW:-%s}' % exe,
        'if [ "$PAR" -gt 1 ]; then',
        '  echo "Running $PAR at a time, $NP processes each."',
        "fi",
        "",
        "correr() {",
        '  cd "$1" || return 1',
...
# Runs every point of this sweep. Generated by Olla-DFT.
#
#     python run.py        one after another
#     python run.py 4      four at a time
#
# Does the same as run.sh and works alike on Linux, macOS and Windows: it does
# not need bash, nor xargs, nor binaries named .x. If it does not find
# MPI it runs in serial, which on a laptop is usually the right thing anyway.
import os
import shutil
import subprocess
import sys
...
def binario(nombre):
    # the executable, trying .x and .exe: on Windows it is pw.exe
    base = nombre
    for suf in (".x", ".exe"):
...
def main():
    ...
    pw = binario(PW)
    if pw is None:
        print("Cannot find the executable '%s'." % PW)
        print("Put it in the PATH, or pass it like this:  PW=/path/to/pw.x python run.py")
        return 2
    np_ = max(1, int(os.environ.get("NPROC", nucleos() // max(par, 1))))
    if par > 1:
        print("Running %d at a time, %d processes each." % (par, np_))
    if par <= 1:
    ...
    print("Sweep finished. %d of %d failed." % (malos, len(CARPETAS)))
...
def missing_pseudo_warning(common: dict) -> str:
    if not common["missing"]:
        return ""
    return (
        "ATTENTION: pseudopotentials are missing for "
        + ", ".join(common["missing"])
        + f"\nLook for them in {common['pseudo_dir']} or change the path with:\n"
        "  olla-dft config set pseudo_dir /path/to/pseudos"
    )
